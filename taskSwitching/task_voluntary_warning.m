...
try
        ...
        elseif keyCode(startKey)
            ...
            for iTrial = 1: totalTrials % loop through the total trials
                ...
                if volTrialRule == 3
                    ...
                    Screen('TextSize', w, 80); % set text size to the fixation length
                    Screen('TextFont', w, 'Noto Sans Mono');
                    DrawFormattedText(w, targetTrial, 'center', centerY-stimPosition(trialRule), grey);
                    DrawFormattedText(w, nonTargetTrial, 'center', centerY-stimPosition(nonTrialRule), grey);

                    
                elseif nonTrialVisibility == 0
                    distractor(end+1)   = 0;
                    volition(end+1)     = 0;
                    Screen('TextSize', w, 80); % set text size to the fixation length
                    Screen('TextFont', w, 'Noto Sans Mono');
                    DrawFormattedText(w, nonTargetTrial, 'center', centerY-stimPosition(nonTrialRule), black);
    ...
    %% Logging responses
    data = table();
    columnNames = {'TrialNo', 'TrialRule', 'TargetStimulus', 'NonTargetStimulus', 'Distractor', 'VoluntaryTrial','StimulusName', 'TagetType'...
        'StimulusOnset', 'KeyPressed', 'ResponseTime', 'MotorSwitch', 'Answer', 'ReactionTime'};
    variableName = {trialNo', rule', targett_trial',  non_targett_trial', distractor', volition', stimulusName', targetType', stimulusOnsetTime', ...
    ...
    % saving files
    cd '/media/rajat/Rasendisk/lab/PhD/cogFlex/pilotBehavior/voluntaryTrials/'
